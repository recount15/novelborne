...
_ASK_RULE_FILES = ("state_memory.md", "golden_finger.md", "worldbook.md")
_ASK_RULE_CHAR_BUDGET = 3000
_ASK_STATE_CHAR_BUDGET = 2000


def _ask_rules_corpus(enhanced: bool) -> str:
    """组装问答语料：运行时规则 + rules 目录其余规则文档，逐份截断。"""
    parts = []
    base = (fe.load_runtime_rules(enhanced) or "").strip()
    if base:
        parts.append(base[:_ASK_RULE_CHAR_BUDGET])
    for name in _ASK_RULE_FILES:
        ...
        try:
            ...
            continue
        if text:
            parts.append(f"# {name}\n" + text[:_ASK_RULE_CHAR_BUDGET])
    return "\n\n".join(parts)


def _ask_state_summary(state: dict[str, Any]) -> str:
    ...
    return json.dumps(summary, ensure_ascii=False, default=str)[:_ASK_STATE_CHAR_BUDGET]
    ...
    try:
        ...
        if engine.cheat_code.is_armed(state):
            ...
            if not clean:
                raise HTTPException(
                    status_code=400,
                    detail="愿望在剥离机制诉求后为空——铁律只能修改世界观与剧情，"
                           "不能修改游戏机制。")
            ...
            try:
                ...
                raise HTTPException(status_code=502, detail=f"愿望实现失败：{exc}") from exc
            granted = str(granted or "").strip()
            if not granted:
                raise HTTPException(
                    status_code=502,
                    detail="愿望实现失败：模型返回为空。本次不消耗次数，请重试。")
            if session.api_key and session.api_key in granted:
                granted = granted.replace(session.api_key, "***")
        ...
        if engine.cheat_code.is_relay_active(state):
            ...
            if not clean:
                raise HTTPException(
                    status_code=400,
                    detail="增补在剥离机制诉求后为空——增补只能修改世界观与剧情，"
                           "不能修改游戏机制。")
            relay_prompt = (
            ...
            try:
                ...
                raise HTTPException(status_code=502, detail=f"增补生成失败：{exc}") from exc
            fact_text = str(fact_text or "").strip()
            if session.api_key and session.api_key in fact_text:
                fact_text = fact_text.replace(session.api_key, "***")
        ...
        system_prompt = (
            ...
            + _ask_rules_corpus(enhanced)
            + "\n\n# 当前对局状态摘要\n" + _ask_state_summary(state)
        )
        messages = [
        ...
        if session.api_key and session.api_key in answer:
            answer = answer.replace(session.api_key, "***")
