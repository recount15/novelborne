...
const emit = defineEmits<{ close: []; 'start-from-chapter': [payload: { bookId: string; chapterNo: number }]; 'chat-with-character': [payload: { bookId: string; chapterNo: number; characterId?: string }]; 'reader-event': [event: { type: string; bookId?: string; chapterIndex?: number }] }>()
...
function readerEntry(kind: 'start' | 'chat') {
  ...
  else emit('chat-with-character', payload)
}
