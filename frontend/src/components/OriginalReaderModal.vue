...
import { Anchor, BookOpen, Bookmark, ChevronLeft, ChevronRight, LibraryBig, List, LoaderCircle, Search, Settings2, SlidersHorizontal, Users, X } from 'lucide-vue-next'
import { fetchUserBook, fetchUserBookChapter, fetchUserBookChapterInsight, listUserBooks, searchBookOccurrences } from '../api'
import { prepareReaderText, originalRangeToDisplay, readerTextParts, readerChapterCharCount, readerOccurrenceLabel, type TextRange } from '../utils/readerText'
import type { UserBookChapter, UserBookChapterInsight, UserBookDetail, UserBookMeta } from '../types'
import { useChapterSearch, useReaderState, type ReaderBookmark } from '../composables/useReaderState'
...
// Initial entry hint only; resolve against the server book list, never a title or path.
const props = defineProps<{ initialBookId?: string }>()
const emit = defineEmits<{ close: []; 'start-from-chapter': [payload: { bookId: string; chapterNo: number }]; 'chat-with-character': [payload: { bookId: string; chapterNo: number; view: 'original'; characterId?: string }]; 'reader-event': [event: { type: string; bookId?: string; chapterIndex?: number }] }>()
type Drawer = 'books' | 'chapters' | 'bookmarks' | 'anchors' | 'characters' | null
const { settings, progress, bookmarksFor, saveProgress, addBookmark, updateBookmark, removeBookmark, resetSettings } = useReaderState()
...
const bookRequest = ref(0); const listRequest = ref(0); const chapterRequest = ref(0); let bodyOverflow = ''; let saveTimer: ReturnType<typeof setTimeout> | undefined
...
const preparedText = computed(() => prepareReaderText(activeChapter.value?.text || '', activeChapter.value?.title))
const displayText = computed(() => preparedText.value.text)
const search = useChapterSearch(() => displayText.value)
const searchScope = ref<'chapter' | 'book'>('chapter')
const searchMode = ref<'exact' | 'fuzzy'>('exact')
const ignorePunctuation = ref(false)
const searchField = ref<HTMLInputElement | null>(null)
type SearchResponse = Awaited<ReturnType<typeof searchBookOccurrences>>
type SearchHit = SearchResponse['hits'][number]
const bookResults = ref<SearchResponse | null>(null)
const searchBusy = ref(false); const searchError = ref('')
const selectedOccurrence = ref<TextRange | null>(null)
let searchRequest = 0
function resetBookSearch() { ++searchRequest; bookResults.value = null; searchBusy.value = false; searchError.value = ''; selectedOccurrence.value = null }
watch([() => search.query.value, searchScope, searchMode, ignorePunctuation, () => activeBook.value?.book_id], resetBookSearch, { flush: 'sync' })
watch(() => activeChapter.value, () => { selectedOccurrence.value = null })
watch(searchOpen, async open => { if (open) { await nextTick(); searchField.value?.focus() } })
async function runBookSearch(page = 1) {
  const bookId = activeBook.value?.book_id; const query = search.query.value.trim()
  if (!bookId || !query || bookLoading.value) return
  const request = ++searchRequest
  searchBusy.value = true; searchError.value = ''; selectedOccurrence.value = null
  try {
    const result = await searchBookOccurrences(bookId, { query, mode: searchMode.value, page, pageSize: 20, ignorePunctuation: ignorePunctuation.value })
    if (request !== searchRequest || activeBook.value?.book_id !== bookId) return
    if (bookResults.value && (bookResults.value.source_hash !== result.source_hash || bookResults.value.index_version !== result.index_version) && page !== 1) {
      bookResults.value = null; searchError.value = '原文索引已更新，请重新搜索。'; return
    }
    bookResults.value = result
  } catch (e) { if (request === searchRequest) { bookResults.value = null; searchError.value = e instanceof Error ? e.message : '全书搜索失败，请重试' } }
  finally { if (request === searchRequest) searchBusy.value = false }
}
async function jumpOccurrence(hit: SearchHit) {
  const bookId = activeBook.value?.book_id; const request = searchRequest
  if (!bookId || chapterLoading.value || bookLoading.value || windowMoving.value) return
  if (!activeBook.value?.chapters.some(ch => ch.index === hit.chapter_no)) { searchError.value = '搜索章节不在当前目录中，请重新搜索。'; return }
  await selectChapter(hit.chapter_no, undefined, 'start')
  if (request !== searchRequest || activeBook.value?.book_id !== bookId || activeChapter.value?.index !== hit.chapter_no || error.value) return
  const range = originalRangeToDisplay(preparedText.value.sourceToDisplay, hit.start, hit.end)
  if (!range) { searchError.value = '搜索位置已失效，请重新搜索。'; return }
  selectedOccurrence.value = range
  await nextTick()
  await scrollChapterTo(range.end > range.start ? { displayOffset: range.start } : 'start')
}
async function scrollLocalMatch(delta?: number) {
  if (searchScope.value !== 'chapter' || !search.matches.value.length || chapterLoading.value) return
  if (delta != null) search.nextMatch(delta)
  const chapter = activeChapter.value; const query = search.query.value
  await nextTick()
  const offset = search.matches.value[search.matchIndex.value]
  if (chapter !== activeChapter.value || query !== search.query.value || searchScope.value !== 'chapter' || offset == null) return
  await scrollChapterTo({ displayOffset: offset })
}
watch([search.matches, search.matchIndex], () => { void scrollLocalMatch() }, { flush: 'post' })
function readerEntry(kind: 'start' | 'chat') {
  if (!activeBook.value || !activeChapter.value || chapterLoading.value || bookLoading.value) return
  const payload = { bookId: activeBook.value.book_id, chapterNo: activeChapter.value.index }
  persistPosition()
  if (kind === 'start') emit('start-from-chapter', payload)
  else emit('chat-with-character', { ...payload, view: 'original' })
}
...
const highlightedText = computed(() => readerTextParts(displayText.value, searchScope.value === 'chapter' ? search.ranges.value : selectedOccurrence.value ? [selectedOccurrence.value] : []))
...
const readingProgress = computed(() => activeBook.value && activeChapter.value ? Math.round((progress.value[activeBook.value.book_id]?.chapterIndex === activeChapter.value.index ? progress.value[activeBook.value.book_id]!.scrollRatio : 0) * 100) : 0)
...
function toggleDrawer(kind: Exclude<Drawer, null>) {
  ...
  // 打开锚点/人物抽屉时，若本章资料尚未蒸馏出来，立即探测一次（不等下个轮询周期）。
  if (drawer.value === kind && (kind === 'anchors' || kind === 'characters')
    && !insightReady(chapterInsight.value) && activeBook.value && activeChapter.value)
    scheduleInsightPolling(activeBook.value.book_id, activeChapter.value.index, 0)
}
...
type ScrollTarget = 'start' | 'end' | { ratio: number } | { anchorText: string; basis?: 'top' | 'bottom' } | { displayOffset: number }
      ...
      if (textEl && textEl.textContent) {
        ...
        if (at >= 0) {
          ...
          if (anchorNode) {
            ...
            const basis = 'displayOffset' in target || target.basis === 'top' ? 'top' : 'bottom'
...
async function loadBooks(preferredBookId?: string) { const request = ++listRequest.value; booksLoading.value = true; error.value = ''; try { const loaded = await listUserBooks(); if (request !== listRequest.value) return; books.value = loaded; const target = [preferredBookId, activeBook.value?.book_id, loaded[0]?.book_id].find(id => id && loaded.some(book => book.book_id === id)); if (target && loaded.some(b => b.book_id === target)) await selectBook(target) } catch (e) { if (request === listRequest.value) error.value = e instanceof Error ? e.message : '读取原著列表失败' } finally { if (request === listRequest.value) booksLoading.value = false } }
async function selectBook(bookId: string) { if (bookLoading.value || chapterLoading.value) return; persistPosition(); resetBookSearch(); search.query.value = ''; const request = ++bookRequest.value; bookLoading.value = true; error.value = ''; try { const book = await fetchUserBook(bookId); if (request !== bookRequest.value) return; activeBook.value = book; activeChapter.value = null; prevChapter.value = null; nextChapter.value = null; chapterInsight.value = null; stopInsightPolling(); const saved = storedProgress(bookId); const index = book.chapters.find(chapter => chapter.index === saved?.chapterIndex)?.index ?? book.chapters[0]?.index; if (index != null) { // 恢复上次阅读：章节号 + 章内滚动比例
    await selectChapter(index, book, saved?.chapterIndex === index && saved.scrollRatio != null ? { ratio: saved.scrollRatio } : 'start') } emit('reader-event', { type: 'book-selected', bookId }) } catch (e) { if (request === bookRequest.value) error.value = e instanceof Error ? e.message : '读取章节目录失败' } finally { if (request === bookRequest.value) bookLoading.value = false } }
...
async function selectChapter(index: number, bookOverride?: UserBookDetail, scrollTarget: ScrollTarget | null = 'start') {
  ...
  try {
    ...
    if (insightReady(insight)) stopInsightPolling()
    else scheduleInsightPolling(book.book_id, index)
    drawer.value = null
...
// —— 蒸馏进行中的章级资料拾取 ——
// 当前章锚点/活跃人物可能尚未蒸馏出来：insight 为空时低频轮询本章，
// 锚点文件落盘后自动刷新抽屉面板（每章锚点只写一次，出现即终态）。
const INSIGHT_POLL_INTERVAL = 15000
let insightPollTimer: ReturnType<typeof setTimeout> | undefined
let insightPollBook: string | null = null
let insightPollChapter = 0
function insightReady(insight: UserBookChapterInsight | null): boolean {
  return !!insight && (!!insight.anchor || (insight.characters?.length ?? 0) > 0)
}
function stopInsightPolling() { clearTimeout(insightPollTimer); insightPollTimer = undefined; insightPollBook = null }
function scheduleInsightPolling(bookId: string, chapterIndex: number, delay = INSIGHT_POLL_INTERVAL) {
  stopInsightPolling()
  insightPollBook = bookId
  insightPollChapter = chapterIndex
  insightPollTimer = setTimeout(async () => {
    const book = insightPollBook
    const index = insightPollChapter
    if (!book || activeBook.value?.book_id !== book || activeChapter.value?.index !== index) { stopInsightPolling(); return }
    try {
      const insight = await fetchUserBookChapterInsight(book, index)
      if (activeBook.value?.book_id !== book || activeChapter.value?.index !== index) { stopInsightPolling(); return }
      if (insightReady(insight)) { chapterInsight.value = insight; stopInsightPolling() }
      else scheduleInsightPolling(book, index)
    } catch { if (insightPollBook === book) scheduleInsightPolling(book, index) }
  }, delay)
}
...
function runSearch() { if (searchScope.value === 'book') void runBookSearch(); else void scrollLocalMatch(1) }
...
onMounted(() => { bodyOverflow = document.body.style.overflow; document.body.style.overflow = 'hidden'; window.addEventListener('keydown', onKeydown); void loadBooks(props.initialBookId) })
onBeforeUnmount(() => { ++searchRequest; ++bookRequest.value; ++chapterRequest.value; ++listRequest.value; clearTimeout(loadWatchdog); persistPosition(); clearTimeout(saveTimer); stopInsightPolling(); document.body.style.overflow = bodyOverflow; window.removeEventListener('keydown', onKeydown); if (scrollEndTimer) clearTimeout(scrollEndTimer); clearTimeout(suppressTimer) })
</script>
...
<template>
  <div class="reader-overlay" role="dialog" aria-modal="true" aria-label="我的原著阅读器">
    <section class="reader-shell" :class="`reader-tone-${settings.tone}`" :style="readerStyle">
      <header class="reader-head"><div class="reader-heading"><span class="reader-mark"><BookOpen :size="16" /></span><div class="min-w-0"><p class="reader-overline">本章进度 · {{ readingProgress }}%</p><h2>{{ activeBook?.name || '原著阅读' }}</h2></div></div><div class="reader-actions"><button class="reader-entry" :disabled="!activeChapter || chapterLoading || bookLoading" title="以本章开始前的剧情状态进入故事" @click="readerEntry('start')">从本章开始</button><button class="reader-entry" :disabled="!activeChapter || chapterLoading || bookLoading" title="以本章结束时的人物状态进入对话" @click="readerEntry('chat')">与人物对话</button><button class="reader-tool" :class="{ active: drawer === 'books' }" title="书库" aria-label="书库" @click="toggleDrawer('books')"><LibraryBig :size="17" /></button><button class="reader-tool" :class="{ active: drawer === 'chapters' }" title="目录" aria-label="目录" @click="toggleDrawer('chapters')"><List :size="18" /></button><button class="reader-tool" :class="{ active: drawer === 'bookmarks' }" title="书签" aria-label="书签" @click="toggleDrawer('bookmarks')"><Bookmark :size="17" /></button><button class="reader-tool" :class="{ active: drawer === 'anchors' }" title="本章锚点" aria-label="本章锚点" @click="toggleDrawer('anchors')"><Anchor :size="17" /></button><button class="reader-tool" :class="{ active: drawer === 'characters' }" title="本章活跃人物" aria-label="本章活跃人物" @click="toggleDrawer('characters')"><Users :size="17" /></button><button class="reader-tool" title="添加书签" aria-label="添加书签" @click="createBookmark"><Bookmark :size="16" /></button><button class="reader-tool" :class="{ active: searchOpen }" title="搜索原文" aria-label="搜索原文" @click="searchOpen = !searchOpen"><Search :size="17" /></button><button class="reader-tool" :class="{ active: settingsOpen }" title="阅读设置" aria-label="阅读设置" @click="toggleSettings"><SlidersHorizontal :size="17" /></button><button class="reader-tool reader-close" title="关闭阅读器" aria-label="关闭阅读器" @click="emit('close')"><X :size="18" /></button></div></header>
      <div v-if="searchOpen" class="reader-search">
        <Search :size="15" /><input ref="searchField" v-model="search.query.value" :placeholder="searchScope === 'chapter' ? '搜索当前章节…' : '搜索整本原著…'" aria-label="搜索原文" @keydown.enter.prevent="runSearch" />
        <select v-model="searchScope" aria-label="搜索范围"><option value="chapter">本章</option><option value="book">全书</option></select>
        <template v-if="searchScope === 'chapter'">
          <span aria-live="polite">{{ !search.query.value.trim() ? '输入关键词' : search.matches.value.length ? `${search.matchIndex.value + 1} / ${search.matches.value.length}` : '无结果' }}</span>
          <button aria-label="上一处匹配" :disabled="!search.matches.value.length || chapterLoading" @click="scrollLocalMatch(-1)"><ChevronLeft :size="15" /></button>
          <button aria-label="下一处匹配" :disabled="!search.matches.value.length || chapterLoading" @click="scrollLocalMatch(1)"><ChevronRight :size="15" /></button>
        </template>
        <template v-else>
          <select v-model="searchMode" aria-label="匹配方式"><option value="exact">精确匹配</option><option value="fuzzy">模糊匹配</option></select>
          <label><input v-model="ignorePunctuation" type="checkbox" />忽略标点</label>
          <button :disabled="searchBusy || !search.query.value.trim() || !activeBook || bookLoading" @click="runBookSearch()">{{ searchBusy ? '搜索中…' : '搜索全书' }}</button>
        </template>
      </div>
      <section v-if="searchOpen && searchScope === 'book'" class="reader-search-results scrollbar" aria-label="全书搜索结果" :aria-busy="searchBusy">
        <p v-if="searchError" role="alert" class="reader-error">{{ searchError }}</p>
        <p v-else-if="searchBusy" role="status">正在搜索原文…</p>
        <template v-else-if="bookResults">
          <div class="reader-result-summary" aria-live="polite"><span>{{ bookResults.total_hits }} 处匹配 · 第 {{ bookResults.page }} 页</span><button :disabled="bookResults.page <= 1" @click="runBookSearch(bookResults.page - 1)">上一页</button><button :disabled="!bookResults.has_more" @click="runBookSearch(bookResults.page + 1)">下一页</button></div>
          <p v-if="!bookResults.hits.length">没有找到匹配原文，试试其他关键词或匹配方式。</p>
          <button v-for="(hit, hitIndex) in bookResults.hits" :key="hit.hit_id" class="reader-result" :aria-label="readerOccurrenceLabel(hit, (bookResults.page - 1) * bookResults.page_size + hitIndex + 1)" :title="readerOccurrenceLabel(hit, (bookResults.page - 1) * bookResults.page_size + hitIndex + 1)" :disabled="chapterLoading || bookLoading || windowMoving" @click="jumpOccurrence(hit)"><small>第 {{ hit.chapter_no }} 章 · {{ hit.match_type === 'exact' ? '精确' : '模糊' }}匹配</small><span>{{ hit.excerpt }}</span></button>
        </template>
        <p v-else>输入原文关键词，按 Enter 搜索全书。结果按原文位置定位，不使用 AI 推测。</p>
      </section>
      <div ref="readerStage" class="reader-stage"><Transition name="reader-drawer"><aside v-if="drawer" class="reader-drawer scrollbar" :aria-label="drawer"><template v-if="drawer === 'books'"><div class="drawer-head"><strong>书库</strong><span>{{ books.length }} 本</span></div><div v-if="booksLoading" class="drawer-loading"><LoaderCircle class="animate-spin" :size="18" /> 正在整理书库</div><button v-for="book in books" :key="book.book_id" class="reader-book" :class="{ active: activeBook?.book_id === book.book_id }" @click="selectBook(book.book_id)"><BookOpen :size="15" /><span><strong>{{ book.name }}</strong><small>{{ book.chapter_count }} 章 · {{ book.source_chars.toLocaleString() }} 字</small></span></button></template><template v-else-if="drawer === 'chapters'"><div class="drawer-head"><strong>目录</strong><span>{{ activeBook?.chapter_count ?? 0 }} 章</span></div><button v-for="chapter in activeBook?.chapters ?? []" :key="chapter.index" class="reader-chapter" :class="{ active: activeChapter?.index === chapter.index }" @click="selectChapter(chapter.index)"><span>{{ String(chapter.index).padStart(2, '0') }}</span><strong>{{ chapter.title }}</strong></button></template><template v-else-if="drawer === 'bookmarks'"><ReaderBookmarksPanel :items="activeBook ? bookmarksFor(activeBook.book_id) : []" :creating="bookmarkCreating" :default-name="activeChapter ? `第 ${activeChapter.index} 章` : '书签'" @create="createBookmark" @confirm-create="commitBookmark" @cancel-create="bookmarkCreating = false" @jump="jumpBookmark" @rename="renameBookmark" @remove="deleteBookmark" /></template><template v-else-if="drawer === 'anchors'"><ReaderAnchorsPanel :anchor="chapterInsight?.anchor ?? null" :loading="insightLoading" :current-chapter-index="activeChapter?.index ?? 0" :status="chapterInsight?.status" :detail="chapterInsight?.detail" /></template><template v-else><ReaderCharactersPanel :characters="chapterInsight?.characters ?? []" :loading="insightLoading" :current-chapter-index="activeChapter?.index ?? 0" /></template></aside></Transition><Transition name="reader-settings"><aside v-if="settingsOpen" class="reader-settings"><div class="drawer-head"><strong><Settings2 :size="15" /> 阅读设置</strong><button @click="resetSettings">恢复默认</button></div><label>字号 <output>{{ settings.fontSize }} px</output><input v-model.number="settings.fontSize" type="range" min="15" max="26" /></label><label>行距 <output>{{ settings.lineHeight.toFixed(1) }}</output><input v-model.number="settings.lineHeight" type="range" min="1.5" max="2.5" step=".1" /></label><label>版心宽度 <output>{{ settings.contentWidth }} px</output><input v-model.number="settings.contentWidth" type="range" min="580" max="880" step="20" /></label><div class="tone-options"><button v-for="tone in ['paper', 'warm', 'night']" :key="tone" :class="{ active: settings.tone === tone }" @click="settings.tone = tone as typeof settings.tone">{{ tone === 'paper' ? '纸白' : tone === 'warm' ? '护眼' : '夜读' }}</button></div></aside></Transition><main ref="readerPage" class="reader-page scrollbar" @scroll.passive="onScroll"><div class="reader-page-inner"><div v-if="(booksLoading || bookLoading || chapterLoading) && !activeChapter" class="reader-state" role="status"><LoaderCircle class="animate-spin" :size="24" /> 正在打开原著</div><template v-else-if="activeChapter"><div v-if="prevChapter && prevPreviewText" ref="prevPreviewRef" class="reader-paper reader-preview-prev"><div class="reader-page-title"><small>第 {{ prevChapter.index }} 章</small><h3>{{ prevChapter.title }}</h3><span>{{ readerChapterCharCount(prevChapter).toLocaleString() }} 字</span></div><div class="reader-text">{{ prevPreviewText }}</div><div class="reader-end">本章完</div></div><div ref="currentContentRef" class="reader-paper" :class="{ loading: chapterLoading }"><div class="reader-page-title"><small>第 {{ activeChapter.index }} 章</small><h3>{{ activeChapter.title }}</h3><span>{{ readerChapterCharCount(activeChapter).toLocaleString() }} 字</span></div><div class="reader-text"><template v-for="(part, i) in highlightedText" :key="i"><mark v-if="part.hit" :data-match-index="part.matchIndex" :class="{ 'current-match': searchScope === 'book' || part.matchIndex === search.matchIndex.value }">{{ part.text }}</mark><template v-else>{{ part.text }}</template></template></div><div class="reader-end">本章完</div></div><div v-if="nextChapter" ref="nextPreviewRef" class="reader-paper reader-preview-next"><div class="reader-page-title"><small>第 {{ nextChapter.index }} 章</small><h3>{{ nextChapter.title }}</h3><span>{{ readerChapterCharCount(nextChapter).toLocaleString() }} 字</span></div><div class="reader-text">{{ nextPreviewText }}</div><div class="reader-end">本章完</div></div></template><div v-else class="reader-state"><BookOpen :size="26" /><strong>从书库打开原著</strong></div><div v-if="error" class="reader-error" role="alert"><p>{{ error }}</p><button :disabled="booksLoading || bookLoading || chapterLoading" @click="loadBooks(props.initialBookId)">重新加载原著</button></div></div></main><button v-if="activeChapter" class="page-scroll page-scroll-up" aria-label="向上翻页" title="向上翻页 (↑ / PageUp)" @click="pageScroll('up')"><ChevronLeft :size="20" style="transform: rotate(90deg)" /></button><button v-if="activeChapter" class="page-scroll page-scroll-down" aria-label="向下翻页" title="向下翻页 (↓ / Space / PageDown)" @click="pageScroll('down')"><ChevronRight :size="20" style="transform: rotate(90deg)" /></button><button v-if="canGoPrevious" class="page-turn page-turn-prev" aria-label="上一章" :disabled="chapterLoading" @click="turn(-1)"><ChevronLeft :size="22" /></button><button v-if="canGoNext" class="page-turn page-turn-next" aria-label="下一章" :disabled="chapterLoading" @click="turn(1)"><ChevronRight :size="22" /></button></div>
...
<style scoped>
.reader-entry,.reader-search-results button{border:0;background:none;color:var(--fe-accent-strong);font-size:12px;padding:6px 9px}.reader-entry{white-space:nowrap}.reader-shell button:disabled{opacity:.4;cursor:not-allowed}.reader-search select{background:var(--fe-panel);color:inherit;border:0;padding:5px}.reader-search label{display:flex;align-items:center;gap:4px;font-size:12px;white-space:nowrap}.reader-search label input{flex:none}.reader-search-results{max-height:28vh;overflow:auto;padding:8px clamp(14px,3vw,34px);background:var(--fe-panel);font-size:12px}.reader-search-results p{margin:6px 0;color:var(--fe-ink-3)}.reader-result-summary{display:flex;align-items:center;gap:12px}.reader-result-summary span{margin-right:auto}.reader-search-results .reader-result{display:block;width:100%;text-align:left;color:var(--fe-ink);padding:9px 0}.reader-result small{display:block;color:var(--fe-ink-3);margin-bottom:3px}.reader-result:hover{background:var(--fe-accent-soft)}.reader-text mark.current-match{background:var(--fe-accent);color:var(--fe-accent-ink)}@media(max-width:900px){.reader-head{flex-wrap:wrap}.reader-actions{flex-wrap:wrap}.reader-heading{flex-basis:100%}}
...
/* Keep reading chrome in the selected reading palette, independently of app theme. */
.reader-tone-paper{--reader-stage-bg:#f6f5f0;--reader-paper-ink:#292c29;--fe-panel:#fffefa;--fe-ink:#292c29;--fe-ink-2:#4d514c;--fe-ink-3:#686c65;--fe-border:#d9dbd2;--fe-accent:#52654c;--fe-accent-strong:#405338;--fe-accent-soft:#e7ecdf;--fe-accent-ink:#fff}
.reader-tone-warm{--fe-panel:#f2ead7;--fe-ink:#443e31;--fe-ink-2:#59513e;--fe-ink-3:#70664f;--fe-border:#cfc4a8;--fe-accent:#766039;--fe-accent-strong:#604a28;--fe-accent-soft:#e3d7b9;--fe-accent-ink:#fff}
.reader-tone-night{color-scheme:dark;--fe-panel:#202627;--fe-ink:#d8d8cc;--fe-ink-2:#c0c4b8;--fe-ink-3:#a3aa9e;--fe-border:#414b45;--fe-accent:#b4c99d;--fe-accent-strong:#c4d9ad;--fe-accent-soft:#354130;--fe-accent-ink:#172014;--fe-danger:#ffb4ab}
.reader-shell{color:var(--fe-ink)}
.reader-head,.reader-footer,.reader-search,.reader-search-results{flex-shrink:0}
.reader-head{display:grid;grid-template-columns:minmax(0,1fr) auto}
.reader-actions{flex-wrap:wrap;justify-content:flex-end;align-items:center}
.reader-tool,.reader-footer button{flex-shrink:0;min-width:36px;min-height:36px}
.reader-shell button:focus-visible,.reader-shell select:focus-visible,.reader-shell input:focus-visible{outline:2px solid var(--fe-accent);outline-offset:2px}
.tone-options button{color:var(--fe-ink)}
.reader-text mark{color:#302a15}
.page-scroll,.page-turn{position:absolute;z-index:2}
.page-scroll-up{top:18px}.page-scroll-down{bottom:18px}
@media(max-width:900px){.reader-head{grid-template-columns:minmax(0,1fr);gap:6px}.reader-actions{justify-content:flex-start;padding-right:42px}.reader-close{position:absolute;right:12px;top:12px}.reader-heading{padding-right:42px}.reader-head h2{max-width:none}.reader-entry{min-height:36px}}
@media(max-width:700px){.reader-head{padding-top:max(10px,env(safe-area-inset-top))}.reader-actions{gap:2px}.reader-tool{width:36px;height:36px}.page-scroll{display:none}.reader-search>input{flex-basis:calc(100% - 30px)}.reader-search label input{flex-basis:auto}.reader-search-results{max-height:24dvh}.reader-page{padding-inline:20px}.reader-result-summary{flex-wrap:wrap}}
</style>
