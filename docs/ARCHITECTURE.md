...
> v2.0.1 权威更新：强化模式已切换为结构化剧情丰度管线；以下“2026-08-30 重构中途快照”中的旧机械门禁、整篇重写和 Phase 进度仅保留为历史测绘。当前实现以本节、`docs/REFACTOR_PLAN.md` 和代码为准。v2.0.2 为修复版：阅读器三章滑动窗口/书签内联 UI/锚点与人物小窗、存读档保真（读档零重蒸馏）、LAN 扫码链路加固，架构与 v2.0.1 一致。
  ...
  → core.server              FastAPI、LAN/QR、会话和静态托管
    → core.app               开局与回合事务状态机
      → core.services        开局、回合、选项、角色、铁律和润色中台
        → core.engine        锚点、角色、剧情丰度、空级批改、并发和确定性规则
          → assets           公开规则、提示词、数据和 18 份内部丰度模板
```
...
→ 定向重填/确定性兜底
→ 代码组装
→ 全局答卷整合润色（无质量门，失败回退组装稿）
→ 角色状态 patch / 任务 / 碎锚 / 宿敌 / 收束结算
→ 事务提交与持久化
```

- 六档剧情丰度：1–3 为小卷族内部模板，4–6 为大卷族内部模板；用户界面统一称“剧情丰度”。
- 质量检测在具体生成空上；整回合只检测格式污染。
- API 动态并发硬上限 10，开局优先于回合，回合优先于后台蒸馏。
- `core/services/turn_pipeline.py` 是新回合中台；`answer_polish_service.py` 负责全局无门禁后处理。
- `opening_service.py` / `opening_distill.py` 负责并行开局蒸馏、首章核验和角色质量门。
- `directives_service.py` / `directives.py` 负责三愿与永久增补的结构化铁律账本。
- `character_service.py` / `character_state_patch.py` 负责证据约束的角色关系入账。
- LAN/二维码支持多网卡选择，二维码保留服务端原始 session ID，扫码恢复同一会话。

## 历史测绘（2026-08-30，供变更追溯）
