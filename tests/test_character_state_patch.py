...
NARRATIVE = (
    "阿岚接过北墙旧册，低声说这是最后一份存档。"
    "林秋守住退路，因此失踪军械的去向曝光，三处暗哨随后被撤换。"
)
...
def entry(name="阿岚", evidence="阿岚接过北墙旧册", delta="升温", summary="因托付旧册而增加信任"):
...
class TestParsePayload(unittest.TestCase):
    def test_good_payload(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry()), ["阿岚", "林秋"], NARRATIVE)
        self.assertEqual(rejected, [])
        self.assertEqual(len(valid), 1)
        self.assertEqual(valid[0]["name"], "阿岚")
    ...
    def test_json_string_and_fenced_input(self):
        ...
        valid, _ = csp.parse_patch_payload(raw, ["阿岚"], NARRATIVE)
        self.assertEqual(len(valid), 1)

    def test_name_not_in_active_members_rejected(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry(name="沈砚")), ["阿岚", "林秋"], NARRATIVE)
        self.assertEqual(valid, [])
        self.assertEqual(len(rejected), 1)
        self.assertIn("沈砚", rejected[0][1] + json.dumps(rejected[0][0], ensure_ascii=False))

    def test_evidence_must_be_exact_substring(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry(evidence="阿岚交出了北墙旧册")), ["阿岚"], NARRATIVE)
        self.assertEqual(valid, [])
    ...
    def test_evidence_with_newline_rejected(self):
        valid, _ = csp.parse_patch_payload(
            payload(entry(evidence="阿岚接过\n北墙旧册")), ["阿岚"], NARRATIVE)
        self.assertEqual(valid, [])

    def test_invalid_delta_rejected(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry(delta="暴涨")), ["阿岚"], NARRATIVE)
        self.assertEqual(valid, [])
    ...
    def test_unknown_field_rejected(self):
        bad = entry()
        bad["mood"] = "开心"
        valid, rejected = csp.parse_patch_payload(payload(bad), ["阿岚"], NARRATIVE)
        self.assertEqual(valid, [])
        self.assertTrue(rejected)

    def test_duplicate_name_rejects_whole_payload(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry(), entry(summary="第二条")), ["阿岚"], NARRATIVE)
        self.assertEqual(valid, [], "重复角色名必须整包拒收")
        self.assertTrue(rejected)

    def test_partial_valid_keeps_good_entries(self):
        valid, rejected = csp.parse_patch_payload(
            payload(entry(), entry(name="林秋", evidence="林秋守住退路", summary="并肩防守")),
            ["阿岚", "林秋"], NARRATIVE)
...
class TestRelationshipRows(unittest.TestCase):
    def test_upsert_preserves_unknown_fields_and_order(self):
        existing = [
            {"name": "沈砚", "note": "旧字段", "relationship_delta": "稳定"},
            {"name": "阿岚", "note": "保留我", "relationship_delta": "稳定", "last_round": 1},
        ]
        rows = csp.build_relationship_rows([entry()], existing, round_no=7)
        self.assertEqual([r["name"] for r in rows], ["沈砚", "阿岚"], "既有顺序不变")
        ...
        self.assertEqual(hit["last_evidence"], "阿岚接过北墙旧册")

    def test_new_name_appended(self):
        rows = csp.build_relationship_rows([entry(name="林秋", evidence="林秋守住退路")],
                                           [{"name": "阿岚"}], round_no=3)
        self.assertEqual([r["name"] for r in rows], ["阿岚", "林秋"])
    ...
    def test_apply_turn_merges_and_keeps_others(self):
        ...
        state["relationships"]["characters"] = [{"name": "沈砚", "relationship_delta": "稳定"}]
        ...
        self.assertIn("沈砚", names, "未命中角色必须仍在")
        self.assertIn("阿岚", names)
...
class TestPatchPrompt(unittest.TestCase):
    def test_prompt_accepts_dicts_and_strings(self):
        text_a = csp.build_patch_prompt(["阿岚", "林秋"], NARRATIVE)
        text_b = csp.build_patch_prompt([{"name": "阿岚"}, {"name": "林秋"}], NARRATIVE)
        for text in (text_a, text_b):
            self.assertIn("阿岚", text)
            self.assertIn("林秋", text)
        self.assertIn("阿岚", text_b)

    def test_prompt_contains_narrative_and_delta_vocab(self):
        text = csp.build_patch_prompt(["阿岚"], NARRATIVE)
